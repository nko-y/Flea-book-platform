...
Page({
  ...
  tap_ch: function (e) {
    ...
    if (this.data.open) {
      ...
      this.setData({
        translate: 'transform: translateX(' + this.data.windowWidth * 1 + 'px)'
      })
      this.data.open = true;
      this.getTabBar().setData({
        translate: 'transform: translateX(' + this.data.windowWidth * 1 + 'px)'
  ...
  saveForm: async function(e){
    ...
    var finprice = e.detail.value.perPrice
    ...
    else if(finprice==""||isNaN(parseFloat(finprice))){
      wx.showModal({
        cancelColor: '提示',
        content:"请输入正确的单价信息",
      })
